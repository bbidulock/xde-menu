 ...
 * styles are contained in the ${_XDE_WM_SYSDIR:-/usr/share/waimea}/styles directory and the  user
 * styles are contained in the ${_XDE_SM_USRDIR:-~/.waimea/styles.
 */
static GList *
...
xde_wkspcs(MenuContext *ctx)
{
	...
	/* FIXME: waimea does not support workspaces command */
	return (text);
}
...
xde_wmspec(MenuContext *ctx)
{
	...
	s = g_strdup_printf("%s%s%s\n", ctx->indent, "[submenu] (Waimea)", icon);
	text = g_list_append(text, s);
	free(icon);
	xde_increase_indent(ctx);
#if 1
