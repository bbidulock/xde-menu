...
Asks a running instance of B<xde-menu> to refresh the meu.  This is
...
at the current pointer posotion, or will popup the menu at the center of
the active screeen.  When multiple monitors are present, the menu will
pop up at the center of the monitor in which the pointer currently
resides (which is used by most tiling winow managers as the "current"
monitor.
...
button that was used to invoke the popup.  Defaults to zero (0).  This
shoule be the button that was used to pop the menu.  A window manaer
...
Provides the X11 server time-stamp of the event that vaused the popup.

=back

=head1 SUPPORTED WINDOW MANAGERS

The following window managers and menuing environments are supported by
B<xde-menu>:
