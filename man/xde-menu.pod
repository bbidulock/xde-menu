...
Useful in conjunction with the B<--menugen> option.

=item B<--notray>

Run without installing the menu in the system tray.
This is the default when the program in invoked as
L<xde-menu-generate(1)>
or when the B<--menugen> option is specified.
