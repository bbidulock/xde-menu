...
=item B<xde-menu>[.I<WMNAME>]B<.keep>: I<INTEGER>

Same as the B<--keep> option to L<xdg-launch(1)>: specifies the maximum
number of commands or application invocations that are kept in the
recent history file.  The minimum value is one, and the maximum is 100.
The default value when unspecified is 10.  Specified as zero (0) when
...
Truth value specifying whether to include unallocated applications in
...
=item F<$XDG_DATA_HOME/recen-applications>
